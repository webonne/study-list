...
分配策略见 [tracks/README.md](README.md)——**别把所有阶段都用 Go 再做一遍**。
...
| # | 指标 | 达标线 | 对应阶段 |
|---|---|---|---|
| G1 | 流式累加正确 | 用 `Accumulate` 拿到的完整消息，与非流式请求的结果结构一致；且 `stream.Err()` 被检查 | 一 |
| G2 | **context 取消真的生效** | 客户端断开后，上游 LLM 调用**在 1s 内停止**（用日志或 token 计数证明没有继续消耗） | 一 / 五 |
| G3 | 并发召回 | 向量 + BM25 并行执行，总耗时 ≈ max(两者) 而非 sum；任一失败时另一路能被取消 | 二 |
| G4 | **race 干净** | `go test -race ./...` **零告警** | 二 / 四 |
| G5 | 并行工具调用 | 一轮内并发执行 ≥ 2 个工具，且**单个工具失败不中断整批** | 四 |
| G6 | **单二进制交付** | MCP Server 交叉编译出 Linux / macOS 二进制，**在没装 Go 的机器上直接运行成功** | 四 |
| G7 | **无 goroutine 泄漏** | 跑完 20 轮长会话后，pprof 里 goroutine 数量**回落到基线附近** | 四 / 五 |
| G8 | schema 正确性 | 每个工具的 `jsonschema` tag 生成的 schema 都**打印检查过**，与预期一致 | 四 |
