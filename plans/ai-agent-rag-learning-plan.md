...
### 阶段一：LLM API 基本功（第 1~2 周）
- [ ] 跑通第一个 API 调用
- [ ] 实现多轮对话 + SSE 流式
- [ ] 实现结构化输出（返回强类型 DTO）
- [ ] 接入 Prompt Caching 并验证命中
- [ ] 完成 Project 0

### 阶段二：RAG（第 3~5 周）
- [ ] 文档解析 + 切块 pipeline
- [ ] 向量库（pgvector）建索引 + 增量更新
- [ ] 混合检索（向量 + BM25 + RRF）
- [ ] 接入 rerank
- [ ] 查询改写（多轮指代消解）
- [ ] 带引用的流式回答
...
- [ ] 手写 tool use 循环
- [ ] 并行工具调用 + 错误处理
- [ ] 上下文管理（compaction / context editing）
- [ ] 写一个 MCP Server
- [ ] 完成 Project 2（含轮数、预算、危险操作护栏）

### 阶段五：生产化（第 11~12 周）
- [ ] 成本监控与优化（缓存命中率、单次成本）
- [ ] 限流、重试、降级
- [ ] Prompt injection 防御 + 权限过滤
- [ ] 全链路可观测 + badcase 回流
- [ ] 写出上线 checklist

### 阶段六：补基础 + 选方向（第 13~14 周）
- [ ] Transformer / Tokenizer / ANN 索引原理
- [ ] 选定一个深入方向并完成一个小实验
